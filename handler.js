...
const Function = new (require('./lib/system/func2'))
const Func = Function
      ...
      if (body && prefix && commands.includes(command) || body && !prefix && commands.includes(command) && setting.noprefix || body && !prefix && commands.includes(command) && env.evaluate_chars.includes(command)) {
         ...
         for (let name in is_commands) {
            ...
            if (cmd.limit && users.limit < 1) {
               client.reply(m.chat, `⚠️ Limit kamu habis an akan di reset pada pukul 00.00\n\nUntuk mendapatkan limit lebih, claim free limit *.claim* atau tingkatkan ke paket premium.`, m).then(() => users.premium = false)
               continue
            }
         ...
         for (let name in is_events) {
            ...
            if (event.limit && !event.game && users.limit < 1 && body && Func.generateLink(body) && Func.generateLink(body).some(v => Func.socmed(v))) return client.reply(m.chat, `⚠️ Limit kamu habis dan akan di reset pada pukul 00.00\n\nUntuk mendapatkan limit lebih, claim free limit *.claim* atau tingkatkan ke paket premium.`, m).then(() => {
               users.premium = false
               users.expired = 0
            })
