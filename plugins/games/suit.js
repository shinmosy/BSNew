exports.run = {
   ...
   async: async (m, {
      ...
      let timeout = 60000
      let poin = Func.randomInt(0, 80000)
      let poin_lose = Func.randomInt(0, 30000)
      if (global.db.users.find(v => v.jid == m.sender).point < 20000) return client.reply(m.chat, Func.texted('bold', `🚩 Point yang kamu miliki tidak cukup untuk bermain game suit, minimal mempunyai 20K point.`), m)
      ...
      if (global.db.users.find(v => v.jid == m.mentionedJid[0]).point < 20000) return client.reply(m.chat, Func.texted('bold', `🚩 Orang yang kamu tantang tidak mempunyai cukup untuk bermain suit.`), m)
